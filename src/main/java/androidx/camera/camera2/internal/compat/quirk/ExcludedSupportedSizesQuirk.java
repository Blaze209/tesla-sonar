package androidx.camera.camera2.internal.compat.quirk;

import android.os.Build;
import android.util.Size;
import androidx.annotation.NonNull;
import androidx.camera.core.impl.m2;
import c0.y0;
import com.google.android.gms.auth.api.proxy.AuthApiStatusCodes;
import java.util.ArrayList;
import java.util.Collections;
import java.util.List;
import org.webrtc.WebrtcBuildVersion;

/* JADX INFO: loaded from: classes.dex */
public class ExcludedSupportedSizesQuirk implements m2 {
    @NonNull
    private List<Size> h(@NonNull String str, int i11, Class<?> cls) {
        ArrayList arrayList = new ArrayList();
        if (str.equals(WebrtcBuildVersion.maint_version) && (i11 == 34 || i11 == 35 || cls != null)) {
            arrayList.add(new Size(720, 720));
            arrayList.add(new Size(400, 400));
        }
        return arrayList;
    }

    @NonNull
    private List<Size> i(@NonNull String str, int i11) {
        ArrayList arrayList = new ArrayList();
        if (str.equals(WebrtcBuildVersion.maint_version) && i11 == 256) {
            arrayList.add(new Size(4160, 3120));
            arrayList.add(new Size(4000, AuthApiStatusCodes.AUTH_API_INVALID_CREDENTIALS));
        }
        return arrayList;
    }

    @NonNull
    private List<Size> j(@NonNull String str, int i11) {
        ArrayList arrayList = new ArrayList();
        if (str.equals(WebrtcBuildVersion.maint_version) && i11 == 256) {
            arrayList.add(new Size(4160, 3120));
            arrayList.add(new Size(4000, AuthApiStatusCodes.AUTH_API_INVALID_CREDENTIALS));
        }
        return arrayList;
    }

    @NonNull
    private List<Size> k(@NonNull String str, int i11) {
        ArrayList arrayList = new ArrayList();
        if (str.equals(WebrtcBuildVersion.maint_version) && i11 == 256) {
            arrayList.add(new Size(9280, 6944));
        }
        return arrayList;
    }

    @NonNull
    private List<Size> l(int i11) {
        ArrayList arrayList = new ArrayList();
        if (i11 == 35) {
            arrayList.add(new Size(3840, 2160));
            arrayList.add(new Size(3264, 2448));
            arrayList.add(new Size(3200, 2400));
            arrayList.add(new Size(2688, 1512));
            arrayList.add(new Size(2592, 1944));
            arrayList.add(new Size(2592, 1940));
            arrayList.add(new Size(1920, 1440));
        }
        return arrayList;
    }

    @NonNull
    private List<Size> m(@NonNull String str, int i11, Class<?> cls) {
        ArrayList arrayList = new ArrayList();
        if (str.equals(WebrtcBuildVersion.maint_version)) {
            if (i11 == 34 || cls != null) {
                arrayList.add(new Size(4128, 3096));
                arrayList.add(new Size(4128, 2322));
                arrayList.add(new Size(3088, 3088));
                arrayList.add(new Size(3264, 2448));
                arrayList.add(new Size(3264, 1836));
                arrayList.add(new Size(2048, 1536));
                arrayList.add(new Size(2048, 1152));
                arrayList.add(new Size(1920, 1080));
                return arrayList;
            }
            if (i11 == 35) {
                arrayList.add(new Size(2048, 1536));
                arrayList.add(new Size(2048, 1152));
                arrayList.add(new Size(1920, 1080));
                return arrayList;
            }
        } else if (str.equals("1") && (i11 == 34 || i11 == 35 || cls != null)) {
            arrayList.add(new Size(2576, 1932));
            arrayList.add(new Size(2560, 1440));
            arrayList.add(new Size(1920, 1920));
            arrayList.add(new Size(2048, 1536));
            arrayList.add(new Size(2048, 1152));
            arrayList.add(new Size(1920, 1080));
        }
        return arrayList;
    }

    @NonNull
    private List<Size> n(@NonNull String str, int i11, Class<?> cls) {
        ArrayList arrayList = new ArrayList();
        if (str.equals(WebrtcBuildVersion.maint_version)) {
            if (i11 == 34 || cls != null) {
                arrayList.add(new Size(4128, 3096));
                arrayList.add(new Size(4128, 2322));
                arrayList.add(new Size(3088, 3088));
                arrayList.add(new Size(3264, 2448));
                arrayList.add(new Size(3264, 1836));
                arrayList.add(new Size(2048, 1536));
                arrayList.add(new Size(2048, 1152));
                arrayList.add(new Size(1920, 1080));
                return arrayList;
            }
            if (i11 == 35) {
                arrayList.add(new Size(4128, 2322));
                arrayList.add(new Size(3088, 3088));
                arrayList.add(new Size(3264, 2448));
                arrayList.add(new Size(3264, 1836));
                arrayList.add(new Size(2048, 1536));
                arrayList.add(new Size(2048, 1152));
                arrayList.add(new Size(1920, 1080));
                return arrayList;
            }
        } else if (str.equals("1") && (i11 == 34 || i11 == 35 || cls != null)) {
            arrayList.add(new Size(3264, 2448));
            arrayList.add(new Size(3264, 1836));
            arrayList.add(new Size(2448, 2448));
            arrayList.add(new Size(1920, 1920));
            arrayList.add(new Size(2048, 1536));
            arrayList.add(new Size(2048, 1152));
            arrayList.add(new Size(1920, 1080));
        }
        return arrayList;
    }

    private static boolean o() {
        return "HUAWEI".equalsIgnoreCase(Build.BRAND) && "HWANE".equalsIgnoreCase(Build.DEVICE);
    }

    private static boolean p() {
        return "OnePlus".equalsIgnoreCase(Build.BRAND) && "OnePlus6".equalsIgnoreCase(Build.DEVICE);
    }

    private static boolean q() {
        return "OnePlus".equalsIgnoreCase(Build.BRAND) && "OnePlus6T".equalsIgnoreCase(Build.DEVICE);
    }

    private static boolean r() {
        return "REDMI".equalsIgnoreCase(Build.BRAND) && "joyeuse".equalsIgnoreCase(Build.DEVICE);
    }

    private static boolean s() {
        return "SAMSUNG".equalsIgnoreCase(Build.BRAND) && "a05s".equalsIgnoreCase(Build.DEVICE) && Build.MODEL.toUpperCase().contains("SM-A057");
    }

    private static boolean t() {
        return "SAMSUNG".equalsIgnoreCase(Build.BRAND) && "J7XELTE".equalsIgnoreCase(Build.DEVICE) && Build.VERSION.SDK_INT >= 27;
    }

    private static boolean u() {
        return "SAMSUNG".equalsIgnoreCase(Build.BRAND) && "ON7XELTE".equalsIgnoreCase(Build.DEVICE) && Build.VERSION.SDK_INT >= 27;
    }

    static boolean v() {
        return p() || q() || o() || u() || t() || r() || s();
    }

    @NonNull
    public List<Size> g(@NonNull String str, int i11) {
        if (p()) {
            return i(str, i11);
        }
        if (q()) {
            return j(str, i11);
        }
        if (o()) {
            return h(str, i11, null);
        }
        if (u()) {
            return n(str, i11, null);
        }
        if (t()) {
            return m(str, i11, null);
        }
        if (r()) {
            return k(str, i11);
        }
        if (s()) {
            return l(i11);
        }
        y0.l("ExcludedSupportedSizesQuirk", "Cannot retrieve list of supported sizes to exclude on this device.");
        return Collections.EMPTY_LIST;
    }
}

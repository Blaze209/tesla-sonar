...
public abstract class h implements g.b {
    ...
    static enum a extends h {
        ...
        private b0 event(net.time4j.engine.g gVar, double d11, double d12, double d13, boolean z11) {
            ...
            double dB0 = ((double) g0VarG.B0()) + ((((double) (z11 ? 6 : 18)) - d14) / 24.0d);
            double dTrueLongitudeOfSunInDegrees = trueLongitudeOfSunInDegrees(dB0);
            double dC = net.time4j.calendar.astro.a.c(Math.toDegrees(Math.atan(Math.tan(Math.toRadians(dTrueLongitudeOfSunInDegrees)) * 0.91764d)));
            double dFloor = (dC + ((Math.floor(dTrueLongitudeOfSunInDegrees / 90.0d) * 90.0d) - (Math.floor(dC / 90.0d) * 90.0d))) / 15.0d;
            double dSin = Math.sin(Math.toRadians(dTrueLongitudeOfSunInDegrees)) * 0.39782d;
            double dCos = Math.cos(Math.asin(dSin));
            double radians = Math.toRadians(d11);
            double dCos2 = (Math.cos(Math.toRadians(d13)) - (dSin * Math.sin(radians))) / (dCos * Math.cos(radians));
            ...
            double d15 = (((degrees / 15.0d) + dFloor) - (dB0 * 0.06571d)) - 6.622d;
            if (Double.compare(0.0d, d15) > 0) {
                d15 += 24.0d;
            } else if (Double.compare(24.0d, d15) <= 0) {
                d15 -= 24.0d;
            }
            long jLongValue = (((Long) g0VarG.s(z.UTC)).longValue() * 86400) + ((long) ((int) Math.floor((d15 - d14) * 3600.0d)));
            fp0.f fVar = fp0.f.UT;
            if (!fp0.d.n().r()) {
                jLongValue += 63072000;
                fVar = fp0.f.POSIX;
            }
            return (b0) b0.l0(Math.round(jLongValue / 60.0d) * 60, fVar).B(b0.f93964o, TimeUnit.MINUTES);
        }

        private double time0(double d11) {
            i0 i0VarT0 = net.time4j.calendar.astro.c.g(d11).i().t0(p.f94822k);
            return ((double) i0VarT0.S().B0()) + (((double) ((Integer) i0VarT0.U().s(h0.f94535z)).intValue()) / 86400.0d);
        }
        ...
        public double rightAscension(double d11) {
            double dTrueLongitudeOfSunInDegrees = trueLongitudeOfSunInDegrees(time0(d11));
            double dC = net.time4j.calendar.astro.a.c(Math.toDegrees(Math.atan(Math.tan(Math.toRadians(dTrueLongitudeOfSunInDegrees)) * 0.91764d)));
            double dFloor = Math.floor(dTrueLongitudeOfSunInDegrees / 90.0d) * 90.0d;
            return (dC + dFloor) - (Math.floor(dC / 90.0d) * 90.0d);
        }
    ...
    static {
        ...
        h hVar = new h("NOAA", 1) { // from class: net.time4j.calendar.astro.h.b
            ...
            private b0 event(boolean z11, net.time4j.engine.g gVar, double d11, double d12, double d13) {
                b0 b0VarE = g.e(gVar, 12, d12, name());
                double dE = net.time4j.calendar.astro.c.e(b0VarE, fp0.f.TT);
                double dLocalHourAngle = localHourAngle(z11, dE, d11, d13);
                if (Double.isNaN(dLocalHourAngle)) {
                    return null;
                }
                double dLocalHourAngle2 = localHourAngle(z11, dE + (dLocalHourAngle / 86400.0d), d11, d13);
                if (Double.isNaN(dLocalHourAngle2)) {
                    return null;
                }
                long jFloor = (long) Math.floor(dLocalHourAngle2);
                TimeUnit timeUnit = TimeUnit.SECONDS;
                return (b0) b0VarE.H(jFloor, timeUnit).H((int) ((dLocalHourAngle2 - jFloor) * 1.0E9d), TimeUnit.NANOSECONDS).B(b0.f93964o, timeUnit);
            }
            ...
            private double localHourAngle(boolean z11, double d11, double d12, double d13) {
                double dLocalHourAngle = localHourAngle(h.toJulianCenturies(d11), d12, d13);
                if (Double.isNaN(dLocalHourAngle)) {
                    return Double.NaN;
                }
                return z11 ? -dLocalHourAngle : dLocalHourAngle;
            }
            ...
            public double equationOfTime(double d11) {
                ...
                return Math.toDegrees(((((Math.sin(radians) * d12) - ((dExcentricity * 2.0d) * dSin)) + ((((dExcentricity * 4.0d) * d12) * dSin) * Math.cos(radians))) - (((d12 * d12) * Math.sin(radians * 2.0d)) / 2.0d)) - ((((5.0d * dExcentricity) * dExcentricity) * Math.sin(radians2 * 2.0d)) / 4.0d)) * 240.0d;
            }
            ...
            private double localHourAngle(double d11, double d12, double d13) {
                ...
                double dDeclinationRad = declinationRad(d11);
                double dCos = (Math.cos(Math.toRadians(d13)) - (Math.sin(dDeclinationRad) * Math.sin(radians))) / (Math.cos(dDeclinationRad) * Math.cos(radians));
        ...
        h hVar2 = new h("CC", 2) { // from class: net.time4j.calendar.astro.h.c
            ...
            private double approxMomentOfDepression(double d11, double d12, double d13, double d14, boolean z11) {
                ...
                double dSineOffset = sineOffset(d11 - d13, d12, d14);
                if (d14 >= 0.0d) {
                    d15 = z11 ? jFloor : 1 + jFloor;
                } else {
                    d15 = jFloor + 0.5d;
                }
                if (Math.abs(dSineOffset) > 1.0d) {
                    dSineOffset = sineOffset(d15 - d13, d12, d14);
                }
                if (Math.abs(dSineOffset) > 1.0d) {
                    return Double.NaN;
                }
                double degrees = (((double) (z11 ? -1 : 1)) * ((((Math.toDegrees(Math.asin(dSineOffset)) / 360.0d) + 0.5d) % 1.0d) - 0.25d)) + jFloor + 0.5d;
            ...
            /* JADX WARN: Multi-variable type inference failed */
            private b0 event(boolean z11, net.time4j.engine.g gVar, double d11, double d12, double d13) {
                double dTransform = z.JULIAN_DAY_NUMBER.transform(gVar.h(), z.UTC) + (z11 ? 0.25d : 0.75d);
                double dJ = (((double) p.b(new BigDecimal(d12)).j()) - (fp0.f.deltaT(g.g(gVar)) - 43200.0d)) / 86400.0d;
                double dMomentOfDepression = momentOfDepression(dTransform, d11, dJ, d13 - 90.0d, z11);
                if (Double.isNaN(dMomentOfDepression)) {
                    return null;
                }
                return (b0) net.time4j.calendar.astro.c.g(dMomentOfDepression - dJ).i().B(b0.f93964o, TimeUnit.SECONDS);
            }
            ...
            private double momentOfDepression(double d11, double d12, double d13, double d14, boolean z11) {
                double dApproxMomentOfDepression = approxMomentOfDepression(d11, d12, d13, d14, z11);
                if (Double.isNaN(dApproxMomentOfDepression)) {
                    return Double.NaN;
                }
                return Math.abs(d11 - dApproxMomentOfDepression) * 86400.0d < 30.0d ? dApproxMomentOfDepression : momentOfDepression(dApproxMomentOfDepression, d12, d13, d14, z11);
            }
            ...
            private double sineOffset(double d11, double d12, double d13) {
                ...
                return (Math.tan(radians) * Math.tan(dDeclinationRad)) + (Math.sin(Math.toRadians(d13)) / (Math.cos(dDeclinationRad) * Math.cos(radians)));
            }
        ...
        h hVar3 = new h("TIME4J", 3) { // from class: net.time4j.calendar.astro.h.d
            ...
            private b0 event(boolean z11, net.time4j.engine.g gVar, double d11, double d12, double d13) {
                ...
                double d14 = 0.0d;
                while (true) {
                    double dLocalHourAngle = localHourAngle(z11, dE + (d14 / 86400.0d), d11, d13);
                    if (Double.isNaN(dLocalHourAngle)) {
                        return null;
                    }
                    if (Math.abs(dLocalHourAngle - d14) < 15.0d) {
                        long jFloor = (long) Math.floor(dLocalHourAngle);
                        TimeUnit timeUnit = TimeUnit.SECONDS;
                        return (b0) b0VarE.H(jFloor, timeUnit).H((int) ((dLocalHourAngle - jFloor) * 1.0E9d), TimeUnit.NANOSECONDS).B(b0.f93964o, timeUnit);
                    }
                    d14 = dLocalHourAngle;
                }
            }
            ...
            public double getGeodeticAngle(double d11, int i11) {
                ...
                double radians = Math.toRadians(d11);
                double dCos = Math.cos(radians) * h.EQUATORIAL_RADIUS;
                double dSin = Math.sin(radians) * h.POLAR_RADIUS;
                double dSqrt = 4.0680631590769E13d / Math.sqrt((dCos * dCos) + (dSin * dSin));
                return Math.toDegrees(Math.acos(dSqrt / (((double) i11) + dSqrt)));
            }
        ...
        DG_X = new int[]{403406, 195207, 119433, 112392, 3891, 2819, 1721, 660, 350, 334, EnumC4419g.SDK_ASSET_ICON_CHECKMARK_GRAY_VALUE, EnumC4419g.SDK_ASSET_HEADER_ENABLE_TRANSFERS_VALUE, EnumC4419g.SDK_ASSET_ILLUSTRATION_PLAID_OVERLAY_ACCOUNT_NUMBER_CONFIRMED_VALUE, EnumC4419g.SDK_ASSET_ILLUSTRATION_INFOCARD_PAYSTUB_VALUE, 158, 132, EnumC4419g.SDK_ASSET_ILLUSTRATION_FORM_VALUE, 114, 99, 93, 86, 78, 72, 68, 64, 46, 38, 37, 32, 29, 28, 27, 27, 25, 24, 21, 21, 20, 18, 17, 14, 13, 13, 13, 12, 10, 10, 10, 10};
    ...
    public static double apparentSolarLongitude(double d11, double d12) {
        ...
        for (int length = DG_X.length - 1; length >= 0; length--) {
            dSin += ((double) DG_X[length]) * Math.sin(Math.toRadians(DG_Y[length] + (DG_Z[length] * d11)));
        }
        double dAberration = (((((36000.76953744d * d11) + 282.7771834d) + ((dSin * 5.729577951308232d) / 1000000.0d)) + aberration(d11)) + d12) / 360.0d;
    ...
    static void nutations(double d11, double[] dArr) {
        ...
        for (int length = TABLE_22A.length - 1; length >= 0; length--) {
            ...
            double d12 = (dArr2[0] * radians) + (dArr2[1] * radians2) + (dArr2[2] * radians3) + (dArr2[3] * radians4) + (dArr2[4] * radians5);
            dSin += Math.sin(d12) * (dArr2[5] + (dArr2[6] * d11));
            dCos += Math.cos(d12) * (dArr2[7] + (dArr2[8] * d11));
        }

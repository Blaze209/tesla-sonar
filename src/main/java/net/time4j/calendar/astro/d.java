...
public enum d {
    ...
    FIRST_QUARTER(90),
    FULL_MOON(180),
    LAST_QUARTER(EnumC4419g.SDK_ASSET_HEADER_RTP_AUTHORIZE_MICRODEPOSITS_VALUE);
    ...
    private static final b0 ZERO_REF = i0.X(2000, 1, 6, 18, 13, 42).O();
    ...
    private int getEstimatedLunations(b0 b0Var) {
        return net.time4j.base.c.g(Math.round((ZERO_REF.I(b0Var, TimeUnit.DAYS) / MEAN_SYNODIC_MONTH) - (((double) this.phase) / 360.0d)));
    }
    ...
    private double periodic24(double d11, double d12, double d13, double d14) {
        ...
        for (int i11 = 23; i11 >= 0; i11--) {
            ...
            dSin += d15 * sin((((double) iArr2[i11]) * d12) + (((double) iArr3[i11]) * d13) + (((double) iArr4[i11]) * d14));
        }
        return dSin;
    }
    ...
    public b0 after(b0 b0Var) {
        ...
        b0 b0VarAtLunation = atLunation(estimatedLunations);
        int i11 = estimatedLunations;
        while (!b0VarAtLunation.g0(b0Var)) {
            i11++;
            b0VarAtLunation = atLunation(i11);
        }
        if (i11 <= estimatedLunations) {
            while (true) {
                i11--;
                b0 b0VarAtLunation2 = atLunation(i11);
                if (!b0VarAtLunation2.g0(b0Var)) {
                    break;
                }
                b0VarAtLunation = b0VarAtLunation2;
            }
        }
        return b0VarAtLunation;
    }

    /* JADX WARN: Multi-variable type inference failed */
    public b0 atLunation(int i11) {
        double d11 = ((double) i11) + (((double) this.phase) / 360.0d);
        ...
        double dSin = ((((MEAN_SYNODIC_MONTH * d11) + 2451550.09766d) + (((((7.3E-10d * d12) - 1.5E-7d) * d12) + 1.5437E-4d) * d13)) - (Math.sin(Math.toRadians((124.7746d - (1.56375588d * d11)) + (((2.15E-6d * d12) + 0.0020672d) * d13))) * 1.7E-4d)) + periodic24(d14, d15, d16, d17);
        if (this == FIRST_QUARTER) {
            dSin += corrQuarter(d14, d15, d16, d17);
        ...
        for (int i12 = 0; i12 < 28; i12 += 2) {
            dSin += dArr[i12 + 1] * Math.sin(Math.toRadians(dArr[i12]));
        }
        return (b0) c.g(dSin).i().B(b0.f93964o, TimeUnit.SECONDS);
    }
    ...
    public b0 atOrAfter(b0 b0Var) {
        ...
        b0 b0VarAtLunation = atLunation(estimatedLunations);
        int i11 = estimatedLunations;
        while (b0VarAtLunation.h0(b0Var)) {
            i11++;
            b0VarAtLunation = atLunation(i11);
        }
        if (i11 <= estimatedLunations) {
    ...
    public b0 before(b0 b0Var) {
        ...
        if (i11 >= estimatedLunations) {
            while (b0VarAtLunation.H(29L, TimeUnit.DAYS).h0(b0Var)) {
                i11++;
                b0 b0VarAtLunation2 = atLunation(i11);
                if (!b0VarAtLunation2.h0(b0Var)) {
                    break;
    ...
    public static double getIllumination(b0 b0Var, int i11) {
        double dB = c.h(b0Var).b();
        double dC = e.c(dB);
        double dB2 = e.b(dB);
        double dA = e.a(dB);
        double d11 = dC * 2.0d;
        double dCos = (cos(((((((180.0d - dC) - (sin(dA) * 6.289d)) + (sin(dB2) * 2.1d)) - (sin(d11 - dA) * 1.274d)) - (sin(d11) * 0.658d)) - (sin(dA * 2.0d) * 0.214d)) - (sin(dC) * 0.11d)) + 1.0d) / 2.0d;

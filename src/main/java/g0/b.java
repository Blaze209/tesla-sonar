...
class b {
    ...
    private static List<j1.c> c(@NonNull List<EncoderProfiles.VideoProfile> list) {
        ...
        for (EncoderProfiles.VideoProfile videoProfile : list) {
            arrayList.add(j1.c.a(videoProfile.getCodec(), videoProfile.getMediaType(), videoProfile.getBitrate(), videoProfile.getFrameRate(), videoProfile.getWidth(), videoProfile.getHeight(), videoProfile.getProfile(), 8, 0, 0));
        }
        return arrayList;
    }
}

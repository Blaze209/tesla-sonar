...
public final class SizeBinder implements ValidationBinder {
    ...
    public FieldValidator<?> bind(Annotation annotation, q fieldType) {
        ...
        if (c.k(dVar, o0.b(String.class))) {
            return new StringSizeValidator(size.min(), size.max());
        }
        return (c.k(dVar, o0.b(Object[].class)) || vn0.a.b(dVar).isArray()) ? new ArraySizeValidator(size.min(), size.max()) : new CollectionSizeValidator(size.min(), size.max());
    }
}

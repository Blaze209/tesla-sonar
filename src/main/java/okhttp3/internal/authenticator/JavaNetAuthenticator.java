...
public final class JavaNetAuthenticator implements Authenticator {
    ...
    private final InetAddress connectToInetAddress(Proxy proxy, HttpUrl httpUrl, Dns dns) {
        ...
        if ((type == null ? -1 : WhenMappings.$EnumSwitchMapping$0[type.ordinal()]) == 1) {
            return (InetAddress) v.o0(dns.lookup(httpUrl.host()));
        }
        SocketAddress socketAddressAddress = proxy.address();
        s.i(socketAddressAddress, "null cannot be cast to non-null type java.net.InetSocketAddress");
        InetAddress address = ((InetSocketAddress) socketAddressAddress).getAddress();
        s.j(address, "address() as InetSocketAddress).address");
        return address;
    }
    ...
    public Request authenticate(Route route, Response response) {
        ...
        for (Challenge challenge : listChallenges) {
            if (t.M("Basic", challenge.scheme(), true)) {
                ...
                if (passwordAuthenticationRequestPasswordAuthentication != null) {
                    ...
                    s.j(userName, "auth.userName");
                    char[] password = passwordAuthenticationRequestPasswordAuthentication.getPassword();
                    s.j(password, "auth.password");

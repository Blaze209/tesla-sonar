...
public final class CacheInterceptor implements Interceptor {
    ...
    public static final class Companion {
        ...
        public final Headers combine(Headers cachedHeaders, Headers networkHeaders) {
            ...
            for (int i11 = 0; i11 < size; i11++) {
                String strName = cachedHeaders.name(i11);
                String strValue = cachedHeaders.value(i11);
                if ((!t.M("Warning", strName, true) || !t.b0(strValue, "1", false, 2, null)) && (isContentSpecificHeader(strName) || !isEndToEnd(strName) || networkHeaders.get(strName) == null)) {
                    builder.addLenient$okhttp(strName, strValue);
                }
            }
            int size2 = networkHeaders.size();
            for (int i12 = 0; i12 < size2; i12++) {
                String strName2 = networkHeaders.name(i12);
                if (!isContentSpecificHeader(strName2) && isEndToEnd(strName2)) {
                    builder.addLenient$okhttp(strName2, networkHeaders.value(i12));
                }
            }
        ...
        private final boolean isContentSpecificHeader(String fieldName) {
            return t.M("Content-Length", fieldName, true) || t.M("Content-Encoding", fieldName, true) || t.M("Content-Type", fieldName, true);
        }

        private final boolean isEndToEnd(String fieldName) {
            return (t.M("Connection", fieldName, true) || t.M("Keep-Alive", fieldName, true) || t.M("Proxy-Authenticate", fieldName, true) || t.M("Proxy-Authorization", fieldName, true) || t.M("TE", fieldName, true) || t.M("Trailers", fieldName, true) || t.M("Transfer-Encoding", fieldName, true) || t.M("Upgrade", fieldName, true)) ? false : true;
        }
    ...
    private final Response cacheWritingResponse(final CacheRequest cacheRequest, Response response) {
        ...
        p0 body = cacheRequest.getBody();
        ResponseBody responseBodyBody = response.body();
        s.h(responseBodyBody);
        final j source = responseBodyBody.getSource();
        final i iVarC = c0.c(body);
        r0 r0Var = new r0() { // from class: okhttp3.internal.cache.CacheInterceptor$cacheWritingResponse$cacheWritingSource$1
            ...
            public long read(h sink, long byteCount) throws IOException {
                s.k(sink, "sink");
                try {
                    long j11 = source.read(sink, byteCount);
                    if (j11 != -1) {
                        sink.t(iVarC.getBufferField(), sink.getSize() - j11, j11);
                        iVarC.b2();
                        return j11;
                    }
        ...
        return response.newBuilder().body(new RealResponseBody(Response.header$default(response, "Content-Type", null, 2, null), response.body().getContentLength(), c0.d(r0Var))).build();
    }

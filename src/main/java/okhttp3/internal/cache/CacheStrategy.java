...
public final class CacheStrategy {
    ...
    public static final class Factory {
        ...
        private final long cacheResponseAge() {
            ...
            if (i11 != -1) {
                jMax = Math.max(jMax, TimeUnit.SECONDS.toMillis(i11));
            }
            long j11 = this.receivedResponseMillis;
            return jMax + (j11 - this.sentRequestMillis) + (this.nowMillis - j11);
        }
        ...
        private final CacheStrategy computeCandidate() {
            ...
            if (!cacheControl2.noCache()) {
                long j11 = millis2 + jCacheResponseAge;
                if (j11 < millis + jComputeFreshnessLifetime) {
                    Response.Builder builderNewBuilder = this.cacheResponse.newBuilder();
                    if (j11 >= jComputeFreshnessLifetime) {

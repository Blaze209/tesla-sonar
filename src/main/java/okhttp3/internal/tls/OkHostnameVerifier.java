...
public final class OkHostnameVerifier implements HostnameVerifier {
    ...
    private final List<String> getSubjectAltNames(X509Certificate certificate, int type) {
        ...
        try {
            ...
            for (List<?> list : subjectAlternativeNames) {
                if (list != null && list.size() >= 2 && s.f(list.get(0), Integer.valueOf(type)) && (obj = list.get(1)) != null) {
    ...
    private final boolean isAscii(String str) {
        return str.length() == ((int) t0.b(str, 0, 0, 3, null));
    }
    ...
    private final boolean verifyIpAddress(String ipAddress, X509Certificate certificate) {
        ...
        Iterator<T> it = subjectAltNames.iterator();
        while (it.hasNext()) {
            if (s.f(canonicalHost, HostnamesKt.toCanonicalHost((String) it.next()))) {
                return true;
            }
        }
        return false;
    ...
    public final List<String> allSubjectAltNames(X509Certificate certificate) {
        ...
        return v.P0(getSubjectAltNames(certificate, 7), getSubjectAltNames(certificate, 2));
    }
    ...
    private final boolean verifyHostname(String hostname, String pattern) {
        if (hostname != null && hostname.length() != 0 && !t.b0(hostname, ".", false, 2, null) && !t.L(hostname, CallerDataConverter.DEFAULT_RANGE_DELIMITER, false, 2, null) && pattern != null && pattern.length() != 0 && !t.b0(pattern, ".", false, 2, null) && !t.L(pattern, CallerDataConverter.DEFAULT_RANGE_DELIMITER, false, 2, null)) {
            if (!t.L(hostname, ".", false, 2, null)) {
                hostname = hostname + CoreConstants.DOT;
            }
            String str = hostname;
            if (!t.L(pattern, ".", false, 2, null)) {
                pattern = pattern + CoreConstants.DOT;
            }
            String strAsciiToLowercase = asciiToLowercase(pattern);
            if (!t.h0(strAsciiToLowercase, Marker.ANY_MARKER, false, 2, null)) {
                return s.f(str, strAsciiToLowercase);
            }
            if (!t.b0(strAsciiToLowercase, "*.", false, 2, null) || t.u0(strAsciiToLowercase, '*', 1, false, 4, null) != -1 || str.length() < strAsciiToLowercase.length() || s.f("*.", strAsciiToLowercase)) {
                return false;
            }
            String strSubstring = strAsciiToLowercase.substring(1);
            s.j(strSubstring, "this as java.lang.String).substring(startIndex)");
            if (!t.L(str, strSubstring, false, 2, null)) {
                return false;
            }
            int length = str.length() - strSubstring.length();
            return length <= 0 || t.B0(str, CoreConstants.DOT, length + (-1), false, 4, null) == -1;
        }
        return false;
    }
}

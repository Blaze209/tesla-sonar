...
final class ResumeAwaitOnCompletion<T> extends JobNode {
    ...
    public void invoke(Throwable cause) {
        ...
        if (state$kotlinx_coroutines_core instanceof CompletedExceptionally) {
            ...
            cancellableContinuationImpl.resumeWith(s.b(t.a(((CompletedExceptionally) state$kotlinx_coroutines_core).cause)));
        } else {
            CancellableContinuationImpl<T> cancellableContinuationImpl2 = this.continuation;
            s.Companion companion2 = s.INSTANCE;
            cancellableContinuationImpl2.resumeWith(s.b(JobSupportKt.unboxState(state$kotlinx_coroutines_core)));
        }
    }
}

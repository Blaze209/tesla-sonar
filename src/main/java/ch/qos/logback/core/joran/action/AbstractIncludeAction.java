...
public abstract class AbstractIncludeAction extends Action {
    ...
    private boolean checkAttributes(Attributes attributes) {
        ...
        if (i11 == 0) {
            str = String.format("One of \"%1$s\", \"%2$s\" or \"%3$s\" attributes must be set.", "file", RESOURCE_ATTR, "url");
        } else {
            if (i11 <= 1) {
                if (i11 == 1) {
                    return true;
            ...
            str = String.format("Only one of \"%1$s\", \"%2$s\" or \"%3$s\" attributes should be set.", "file", RESOURCE_ATTR, "url");
        }

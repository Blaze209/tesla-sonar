...
final class FloatArrayList extends AbstractProtobufList<Float> implements Internal.FloatList, RandomAccess, PrimitiveNonBoxingCollection {
    ...
    public int hashCode() {
        ...
        for (int i11 = 0; i11 < this.size; i11++) {
            iFloatToIntBits = (iFloatToIntBits * 31) + Float.floatToIntBits(this.array[i11]);
        }
    ...
    public Internal.ProtobufList<Float> mutableCopyWithCapacity2(int i11) {
        if (i11 >= this.size) {
            return new FloatArrayList(Arrays.copyOf(this.array, i11), this.size, true);
        }

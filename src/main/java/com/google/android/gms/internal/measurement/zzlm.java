package com.google.android.gms.internal.measurement;

import java.util.logging.Level;
import java.util.logging.Logger;

/* JADX INFO: loaded from: classes5.dex */
public abstract class zzlm extends zzkz {
    public static final /* synthetic */ int zzb = 0;
    private static final Logger zzc = Logger.getLogger(zzlm.class.getName());
    private static final boolean zzd = zzop.zza();
    zzln zza;

    private zzlm() {
        throw null;
    }

    public static int zzA(long j11) {
        return (640 - (Long.numberOfLeadingZeros(j11) * 9)) >>> 6;
    }

    public static int zzB(String str) {
        int length;
        try {
            length = zzos.zzb(str);
        } catch (zzor unused) {
            length = str.getBytes(zzmp.zza).length;
        }
        return zzz(length) + length;
    }

    public static int zzC(zznm zznmVar) {
        int iZzcn = zznmVar.zzcn();
        return zzz(iZzcn) + iZzcn;
    }

    static int zzD(zznm zznmVar, zznx zznxVar) {
        int iZzcd = ((zzks) zznmVar).zzcd(zznxVar);
        return zzz(iZzcd) + iZzcd;
    }

    @Deprecated
    static int zzG(int i11, zznm zznmVar, zznx zznxVar) {
        int iZzz = zzz(i11 << 3);
        return iZzz + iZzz + ((zzks) zznmVar).zzcd(zznxVar);
    }

    public static int zzz(int i11) {
        return (352 - (Integer.numberOfLeadingZeros(i11) * 9)) >>> 6;
    }

    public final void zzE() {
        if (zzy() != 0) {
            throw new IllegalStateException("Did not write as much data as expected.");
        }
    }

    final void zzF(String str, zzor zzorVar) throws zzll {
        zzc.logp(Level.WARNING, "com.google.protobuf.CodedOutputStream", "inefficientWriteStringNoTag", "Converting ill-formed UTF-16. Your Protocol Buffer will not round trip correctly!", (Throwable) zzorVar);
        byte[] bytes = str.getBytes(zzmp.zza);
        try {
            int length = bytes.length;
            zzr(length);
            zzw(bytes, 0, length);
        } catch (IndexOutOfBoundsException e11) {
            throw new zzll(e11);
        }
    }

    public abstract void zza(int i11, int i12);

    public abstract void zzb(int i11, int i12);

    public abstract void zzc(int i11, int i12);

    public abstract void zzd(int i11, int i12);

    public abstract void zze(int i11, long j11);

    public abstract void zzf(int i11, long j11);

    public abstract void zzg(int i11, boolean z11);

    public abstract void zzh(int i11, String str);

    public abstract void zzi(int i11, zzlh zzlhVar);

    public abstract void zzj(zzlh zzlhVar);

    abstract void zzk(byte[] bArr, int i11, int i12);

    abstract void zzl(int i11, zznm zznmVar, zznx zznxVar);

    public abstract void zzm(int i11, zznm zznmVar);

    public abstract void zzn(int i11, zzlh zzlhVar);

    public abstract void zzo(zznm zznmVar);

    public abstract void zzp(byte b11);

    public abstract void zzq(int i11);

    public abstract void zzr(int i11);

    public abstract void zzs(int i11);

    public abstract void zzt(long j11);

    public abstract void zzu(long j11);

    public abstract void zzw(byte[] bArr, int i11, int i12);

    public abstract void zzx(String str);

    public abstract int zzy();

    /* synthetic */ zzlm(byte[] bArr) {
    }
}

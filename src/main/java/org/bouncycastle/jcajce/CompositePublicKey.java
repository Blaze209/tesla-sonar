...
public class CompositePublicKey implements PublicKey {
    ...
    public byte[] getEncoded() {
        ...
        for (int i11 = 0; i11 < this.keys.size(); i11++) {
            aSN1EncodableVector.add(this.algorithmIdentifier.equals((ASN1Primitive) MiscObjectIdentifiers.id_composite_key) ? SubjectPublicKeyInfo.getInstance(this.keys.get(i11).getEncoded()) : SubjectPublicKeyInfo.getInstance(this.keys.get(i11).getEncoded()).getPublicKeyData());
        }
        try {

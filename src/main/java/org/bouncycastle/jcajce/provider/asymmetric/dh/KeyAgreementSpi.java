...
public class KeyAgreementSpi extends BaseAgreementSpi {
    ...
    protected byte[] bigIntToBytes(BigInteger bigInteger) {
        return BigIntegers.asUnsignedByteArray((this.f98844p.bitLength() + 7) / 8, bigInteger);
    }
    ...
    protected void doInitFromKey(Key key, AlgorithmParameterSpec algorithmParameterSpec, SecureRandom secureRandom) throws InvalidKeyException, InvalidAlgorithmParameterException {
        ...
        if (algorithmParameterSpec == null) {
            this.f98844p = dHPrivateKey.getParams().getP();
            this.f98843g = dHPrivateKey.getParams().getG();
        } else if (algorithmParameterSpec instanceof DHParameterSpec) {
            DHParameterSpec dHParameterSpec = (DHParameterSpec) algorithmParameterSpec;
            this.f98844p = dHParameterSpec.getP();
            this.f98843g = dHParameterSpec.getG();
    ...
    protected Key engineDoPhase(Key key, boolean z11) throws InvalidKeyException {
        ...
        if (!dHPublicKey.getParams().getG().equals(this.f98843g) || !dHPublicKey.getParams().getP().equals(this.f98844p)) {
            throw new InvalidKeyException("DHPublicKey not for this KeyAgreement!");
        }
        BigInteger y11 = dHPublicKey.getY();
        if (y11 != null && y11.compareTo(TWO) >= 0) {
            BigInteger bigInteger = this.f98844p;
            ...
            if (y11.compareTo(bigInteger.subtract(bigInteger2)) < 0) {
                ...
                BigInteger bigIntegerModPow = y11.modPow(this.f98845x, this.f98844p);

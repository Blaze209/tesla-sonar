...
abstract class X509CertificateImpl extends X509Certificate implements BCX509Certificate {
    ...
    private void checkSignature(PublicKey publicKey, Signature signature, ASN1Encodable aSN1Encodable, byte[] bArr) throws NoSuchAlgorithmException, SignatureException, InvalidKeyException, CertificateException {
        if (!X509SignatureUtil.areEquivalentAlgorithms(this.f98860c.getSignatureAlgorithm(), this.f98860c.getTBSCertificate().getSignature())) {
        ...
        try {
            ...
            this.f98860c.getTBSCertificate().encodeTo(bufferedOutputStream, "DER");
    ...
    private void doVerify(PublicKey publicKey, SignatureCreator signatureCreator) throws NoSuchAlgorithmException, SignatureException, InvalidKeyException, CertificateException {
        ...
        int i11 = 0;
        if (z11 && X509SignatureUtil.isCompositeAlgorithm(this.f98860c.getSignatureAlgorithm())) {
            List<PublicKey> publicKeys = ((CompositePublicKey) publicKey).getPublicKeys();
            ASN1Sequence aSN1Sequence = ASN1Sequence.getInstance(this.f98860c.getSignatureAlgorithm().getParameters());
            ASN1Sequence aSN1Sequence2 = ASN1Sequence.getInstance(this.f98860c.getSignature().getOctets());
            boolean z12 = false;
        ...
        if (!X509SignatureUtil.isCompositeAlgorithm(this.f98860c.getSignatureAlgorithm())) {
            ...
            if (z11) {
                ...
                if (MiscObjectIdentifiers.id_composite_key.equals((ASN1Primitive) compositePublicKey.getAlgorithmIdentifier())) {
                    ...
                    while (i11 != publicKeys2.size()) {
                        try {
                            checkSignature(publicKeys2.get(i11), signatureCreateSignature, this.f98860c.getSignatureAlgorithm().getParameters(), getSignature());
                            return;
            ...
            checkSignature(publicKey, signatureCreateSignature, this.f98860c.getSignatureAlgorithm().getParameters(), getSignature());
            return;
        }
        ASN1Sequence aSN1Sequence3 = ASN1Sequence.getInstance(this.f98860c.getSignatureAlgorithm().getParameters());
        ASN1Sequence aSN1Sequence4 = ASN1Sequence.getInstance(this.f98860c.getSignature().getOctets());
        boolean z13 = false;
    ...
    public Set getCriticalExtensionOIDs() {
        ...
        Extensions extensions = this.f98860c.getTBSCertificate().getExtensions();
        if (extensions == null) {
            return null;
        }
    ...
    public List getExtendedKeyUsage() throws CertificateParsingException {
        byte[] extensionOctets = getExtensionOctets(this.f98860c, "2.5.29.37");
        if (extensionOctets == null) {
            return null;
        }
        try {
    ...
    public Collection getIssuerAlternativeNames() {
        return getAlternativeNames(this.f98860c, Extension.issuerAlternativeName.getId());
    }

    @Override // java.security.cert.X509Certificate
    public Principal getIssuerDN() {
        return new X509Principal(this.f98860c.getIssuer());
    }

    @Override // java.security.cert.X509Certificate
    public boolean[] getIssuerUniqueID() {
        ASN1BitString issuerUniqueId = this.f98860c.getTBSCertificate().getIssuerUniqueId();
    ...
    public X500Name getIssuerX500Name() {
        return this.f98860c.getIssuer();
    }

    @Override // java.security.cert.X509Certificate
    public X500Principal getIssuerX500Principal() {
        try {
            return new X500Principal(this.f98860c.getIssuer().getEncoded("DER"));
    ...
    public Date getNotAfter() {
        return this.f98860c.getEndDate().getDate();
    }

    @Override // java.security.cert.X509Certificate
    public Date getNotBefore() {
        return this.f98860c.getStartDate().getDate();
    }

    @Override // java.security.cert.Certificate
    public PublicKey getPublicKey() {
        try {
            return BouncyCastleProvider.getPublicKey(this.f98860c.getSubjectPublicKeyInfo());
        } catch (IOException e11) {
            throw Exceptions.illegalStateException("failed to recover public key: " + e11.getMessage(), e11);
        }
    }

    @Override // java.security.cert.X509Certificate
    public BigInteger getSerialNumber() {
        return this.f98860c.getSerialNumber().getValue();
    }
    ...
    public String getSigAlgOID() {
        return this.f98860c.getSignatureAlgorithm().getAlgorithm().getId();
    }
    ...
    public byte[] getSignature() {
        return this.f98860c.getSignature().getOctets();
    }

    @Override // java.security.cert.X509Certificate
    public Collection getSubjectAlternativeNames() {
        return getAlternativeNames(this.f98860c, Extension.subjectAlternativeName.getId());
    }

    @Override // java.security.cert.X509Certificate
    public Principal getSubjectDN() {
        return new X509Principal(this.f98860c.getSubject());
    }

    @Override // java.security.cert.X509Certificate
    public boolean[] getSubjectUniqueID() {
        ASN1BitString subjectUniqueId = this.f98860c.getTBSCertificate().getSubjectUniqueId();
    ...
    public X500Name getSubjectX500Name() {
        return this.f98860c.getSubject();
    }

    @Override // java.security.cert.X509Certificate
    public X500Principal getSubjectX500Principal() {
        try {
            return new X500Principal(this.f98860c.getSubject().getEncoded("DER"));
        } catch (IOException unused) {
    ...
    public byte[] getTBSCertificate() throws CertificateEncodingException {
        try {
            return this.f98860c.getTBSCertificate().getEncoded("DER");
        } catch (IOException e11) {
            throw new CertificateEncodingException(e11.toString());
        }
    }
    ...
    public TBSCertificate getTBSCertificateNative() {
        return this.f98860c.getTBSCertificate();
    }

    @Override // java.security.cert.X509Certificate
    public int getVersion() {
        return this.f98860c.getVersionNumber();
    }

    @Override // java.security.cert.X509Extension
    public boolean hasUnsupportedCriticalExtension() {
        Extensions extensions;
        if (getVersion() != 3 || (extensions = this.f98860c.getTBSCertificate().getExtensions()) == null) {
            return false;
        }
        Enumeration enumerationOids = extensions.oids();
        while (enumerationOids.hasMoreElements()) {
    ...
    public byte[] getExtensionValue(String str) {
        ASN1OctetString extensionValue = getExtensionValue(this.f98860c, str);
        if (extensionValue == null) {
            return null;
        }
        try {
            ...
            throw Exceptions.illegalStateException("error parsing " + e11.getMessage(), e11);
        }
    }

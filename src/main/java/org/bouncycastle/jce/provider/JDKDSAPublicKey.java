...
public class JDKDSAPublicKey implements DSAPublicKey {
    ...
    private void readObject(ObjectInputStream objectInputStream) {
        this.f98885y = (BigInteger) objectInputStream.readObject();
        this.dsaSpec = new DSAParameterSpec((BigInteger) objectInputStream.readObject(), (BigInteger) objectInputStream.readObject(), (BigInteger) objectInputStream.readObject());
    }

    private void writeObject(ObjectOutputStream objectOutputStream) throws IOException {
        objectOutputStream.writeObject(this.f98885y);
    ...
    public byte[] getEncoded() {
        try {
            ...
            return dSAParams == null ? new SubjectPublicKeyInfo(new AlgorithmIdentifier(X9ObjectIdentifiers.id_dsa), new ASN1Integer(this.f98885y)).getEncoded("DER") : new SubjectPublicKeyInfo(new AlgorithmIdentifier(X9ObjectIdentifiers.id_dsa, new DSAParameter(dSAParams.getP(), this.dsaSpec.getQ(), this.dsaSpec.getG())), new ASN1Integer(this.f98885y)).getEncoded("DER");
    ...
    JDKDSAPublicKey(DSAPublicKey dSAPublicKey) {
        this.f98885y = dSAPublicKey.getY();
        this.dsaSpec = dSAPublicKey.getParams();
    }

    JDKDSAPublicKey(DSAPublicKeySpec dSAPublicKeySpec) {
        this.f98885y = dSAPublicKeySpec.getY();
        this.dsaSpec = new DSAParameterSpec(dSAPublicKeySpec.getP(), dSAPublicKeySpec.getQ(), dSAPublicKeySpec.getG());
    }

    JDKDSAPublicKey(SubjectPublicKeyInfo subjectPublicKeyInfo) {
        try {
            this.f98885y = ((ASN1Integer) subjectPublicKeyInfo.parsePublicKey()).getValue();
    ...
    JDKDSAPublicKey(DSAPublicKeyParameters dSAPublicKeyParameters) {
        this.f98885y = dSAPublicKeyParameters.getY();

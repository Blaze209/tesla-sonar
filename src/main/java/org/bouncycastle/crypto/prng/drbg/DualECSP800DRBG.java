...
public class DualECSP800DRBG implements SP80090DRBG {
    ...
    public int generate(byte[] bArr, byte[] bArr2, boolean z11) {
        ...
        for (int i11 = 0; i11 < length2; i11++) {
            ...
            if (length5 > i12) {
                System.arraycopy(byteArray, byteArray.length - i12, bArr, length4, i12);
            } else {
                System.arraycopy(byteArray, 0, bArr, (i12 - byteArray.length) + length4, byteArray.length);
            }
        ...
        if (length4 < bArr.length) {
            ...
            if (length7 > i13) {
                length3 = byteArray2.length - i13;
            } else {
                length4 += i13 - byteArray2.length;
            }
    ...
    public DualECSP800DRBG(DualECPoints[] dualECPointsArr, Digest digest, int i11, EntropySource entropySource, byte[] bArr, byte[] bArr2) {
        ...
        if (entropySource.entropySize() < i11 || entropySource.entropySize() > 4096) {
        ...
        for (int i12 = 0; i12 != dualECPointsArr.length; i12++) {
            if (i11 <= dualECPointsArr[i12].getSecurityStrength()) {
                if (Utils.getMaxSecurityStrength(digest) < dualECPointsArr[i12].getSecurityStrength()) {

...
public class GCMSIVBlockCipher implements AEADBlockCipher {
    ...
    private class GCMSIVHasher {
        ...
        void updateHash(byte b11) {
            byte[] bArr = this.theByte;
            bArr[0] = b11;
            updateHash(bArr, 0, 1);
        }

        void updateHash(byte[] bArr, int i11, int i12) {
            int i13;
    ...
    private void checkAEADStatus(int i11) {
        ...
        if (this.theAEADHasher.getBytesProcessed() - Long.MIN_VALUE > ((long) (MAX_DATALEN - i11)) - Long.MIN_VALUE) {
    ...
    private static void mulX(byte[] bArr) {
        ...
        for (int i12 = 0; i12 < 16; i12++) {
            ...
            i11 = (b11 & 1) == 0 ? 0 : RangingPosition.RSSI_UNKNOWN;
        }
        if (i11 != 0) {
    ...
    public int doFinal(byte[] bArr, int i11) throws InvalidCipherTextException, IOException {
        ...
        System.arraycopy(bArrCalculateTag, 0, bArr, i11 + this.thePlain.size(), 16);
        byte[] bArr2 = this.macBlock;
        System.arraycopy(bArrCalculateTag, 0, bArr2, 0, bArr2.length);
        resetStreams();

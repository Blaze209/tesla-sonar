...
public class RC2WrapEngine implements Wrapper {
    ...
    public void init(boolean z11, CipherParameters cipherParameters) {
        ...
        if (cipherParameters instanceof ParametersWithRandom) {
            ...
            this.f98694sr = parametersWithRandom.getRandom();
            cipherParameters = parametersWithRandom.getParameters();
        } else {
            this.f98694sr = CryptoServicesRegistrar.getSecureRandom();
        }
        if (!(cipherParameters instanceof ParametersWithIV)) {
            ...
            if (this.forWrapping) {
                ...
                this.f98693iv = bArr;
                this.f98694sr.nextBytes(bArr);
                this.paramPlusIV = new ParametersWithIV(this.param, this.f98693iv);
                return;
            }
            return;
        }
        ...
        this.f98693iv = parametersWithIV.getIV();
        this.param = this.paramPlusIV.getParameters();
        if (!this.forWrapping) {
            throw new IllegalArgumentException("You should not supply an IV for unwrapping");
        }
        byte[] bArr2 = this.f98693iv;
    ...
    public byte[] unwrap(byte[] bArr, int i11, int i12) throws InvalidCipherTextException {
        ...
        ParametersWithIV parametersWithIV = new ParametersWithIV(this.param, this.f98693iv);
        this.paramPlusIV = parametersWithIV;
    ...
    public byte[] wrap(byte[] bArr, int i11, int i12) {
        ...
        byte[] bArr6 = this.f98693iv;
        int length2 = bArr6.length + length;
        byte[] bArr7 = new byte[length2];
        System.arraycopy(bArr6, 0, bArr7, 0, bArr6.length);
        System.arraycopy(bArr5, 0, bArr7, this.f98693iv.length, length);

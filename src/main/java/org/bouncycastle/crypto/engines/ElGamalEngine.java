...
public class ElGamalEngine implements AsymmetricBlockCipher {
    ...
    public byte[] processBlock(byte[] bArr, int i11, int i12) {
        ...
        while (true) {
            ...
            if (!bigIntegerCreateRandomBigInteger.equals(ZERO) && bigIntegerCreateRandomBigInteger.compareTo(p11.subtract(TWO)) <= 0) {
                break;
            }
        }
        BigInteger bigIntegerModPow = this.key.getParameters().getG().modPow(bigIntegerCreateRandomBigInteger, p11);
        BigInteger bigIntegerMod = bigInteger.multiply(elGamalPublicKeyParameters.getY().modPow(bigIntegerCreateRandomBigInteger, p11)).mod(p11);

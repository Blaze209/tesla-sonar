...
public class HPKE {
    ...
    public HPKE(byte b11, short s11, short s12, short s13) {
        this.mode = b11;
        this.kemId = s11;
        this.kdfId = s12;
        this.aeadId = s13;
        this.hkdf = new HKDF(s12);
        this.dhkem = new DHKEM(s11);
        this.Nk = s13 == 1 ? (short) 16 : (short) 32;
    }

    private void VerifyPSKInputs(byte b11, byte[] bArr, byte[] bArr2) {
        ...
        if (!zAreEqual && b11 % 2 == 0) {
            throw new IllegalArgumentException("PSK input provided when not needed");
        }
        if (zAreEqual && b11 % 2 == 1) {
            throw new IllegalArgumentException("Missing required PSK input");
        }
    }

    private HPKEContext keySchedule(byte b11, byte[] bArr, byte[] bArr2, byte[] bArr3, byte[] bArr4) {
        VerifyPSKInputs(b11, bArr3, bArr4);
        ...
        byte[] bArrConcatenate2 = Arrays.concatenate(new byte[]{b11}, this.hkdf.LabeledExtract(null, bArrConcatenate, "psk_id_hash", bArr4), this.hkdf.LabeledExtract(null, bArrConcatenate, "info_hash", bArr2));
        byte[] bArrLabeledExtract = this.hkdf.LabeledExtract(bArr, bArrConcatenate, "secret", bArr3);
        byte[] bArrLabeledExpand = this.hkdf.LabeledExpand(bArrLabeledExtract, bArrConcatenate, Action.KEY_ATTRIBUTE, bArrConcatenate2, this.Nk);

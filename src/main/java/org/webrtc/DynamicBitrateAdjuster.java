...
class DynamicBitrateAdjuster extends BaseBitrateAdjuster {
    ...
    public void reportEncodedFrame(int i11) {
        ...
        double d12 = this.deviationBytes + (((double) i11) - ((((double) i12) / BITS_PER_BYTE) / d11));
        this.deviationBytes = d12;
        this.timeSinceLastAdjustmentMs += 1000.0d / d11;
        double d13 = ((double) i12) / BITS_PER_BYTE;
        double d14 = BITRATE_ADJUSTMENT_SEC * d13;
        double dMin = Math.min(d12, d14);
        this.deviationBytes = dMin;
        double dMax = Math.max(dMin, -d14);
        this.deviationBytes = dMax;
        if (this.timeSinceLastAdjustmentMs <= 3000.0d) {
            return;
        }
        if (dMax > d13) {
            int i13 = this.bitrateAdjustmentScaleExp - ((int) ((dMax / d13) + 0.5d));
            ...
            double d15 = -d13;
            if (dMax < d15) {
                int i14 = this.bitrateAdjustmentScaleExp + ((int) (((-dMax) / d13) + 0.5d));
    ...
    public void setTargets(int i11, double d11) {
        int i12 = this.targetBitrateBps;
        if (i12 > 0 && i11 < i12) {
            this.deviationBytes = (this.deviationBytes * ((double) i11)) / ((double) i12);
        }

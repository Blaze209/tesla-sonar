...
public final class RenderSynchronizer {
    ...
    public RenderSynchronizer(float f11) {
        this.lock = new Object();
        this.listeners = new CopyOnWriteArrayList();
        this.targetFrameIntervalNanos = Math.round(TimeUnit.SECONDS.toNanos(1L) / f11);
        Handler handler = new Handler(Looper.getMainLooper());
        this.mainThreadHandler = handler;
        handler.post(new Runnable() { // from class: org.webrtc.p0
            @Override // java.lang.Runnable
            public final void run() {
                this.f99934a.lambda$new$0();
            }
        });
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public void onDisplayRefreshCycleBegin(long j11) {
        synchronized (this.lock) {
            try {
                ...
                this.choreographer.postFrameCallback(new o0(this));
                long j12 = j11 - this.lastOpenedTimeNanos;
                long j13 = j11 - this.lastRefreshTimeNanos;
                this.lastRefreshTimeNanos = j11;
                if (Math.abs(j12 - this.targetFrameIntervalNanos) < Math.abs((j12 - this.targetFrameIntervalNanos) + j13)) {
                    this.lastOpenedTimeNanos = j11;
                    openRenderWindow();
            ...
            try {
                if (!this.isListening) {
                    ...
                    this.mainThreadHandler.post(new Runnable() { // from class: org.webrtc.n0
                        @Override // java.lang.Runnable
                        public final void run() {
                            this.f99922a.lambda$registerListener$1();
                        }
                    });
                }
            } catch (Throwable th2) {

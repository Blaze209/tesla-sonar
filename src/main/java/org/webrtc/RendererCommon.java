...
public class RendererCommon {
    ...
    public static class VideoLayoutMeasure {
        ...
        public Point measure(int i11, int i12, int i13, int i14) {
            ...
            float f11 = i13 / i14;
            Point displaySize = RendererCommon.getDisplaySize(((f11 > 1.0f ? 1 : (f11 == 1.0f ? 0 : -1)) > 0) == (((float) defaultSize) / ((float) defaultSize2) > 1.0f) ? this.visibleFractionMatchOrientation : this.visibleFractionMismatchOrientation, f11, defaultSize, defaultSize2);
    ...
    public static Point getDisplaySize(float f11, float f12, int i11, int i12) {
        return (f11 == BitmapDescriptorFactory.HUE_RED || f12 == BitmapDescriptorFactory.HUE_RED) ? new Point(i11, i12) : new Point(Math.min(i11, Math.round((i12 / f11) * f12)), Math.min(i12, Math.round((i11 / f11) / f12)));
    }
}

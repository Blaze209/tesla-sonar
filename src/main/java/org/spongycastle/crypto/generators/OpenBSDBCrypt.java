...
public class OpenBSDBCrypt {
    ...
    public static boolean checkPassword(String str, char[] cArr) {
        ...
        try {
            int i11 = Integer.parseInt(str.substring(4, 6));
            if (i11 >= 4 && i11 <= 31) {
                if (cArr != null) {
                    return str.equals(generate(cArr, decodeSaltString(str.substring(str.lastIndexOf(36) + 1, str.length() - 31)), i11));
                }
    ...
    private static String encodeData(byte[] bArr) {
        ...
        for (int i11 = 0; i11 < length; i11 += 3) {
            ...
            byteArrayOutputStream.write(bArr3[((i13 << 2) | ((b11 & 255) >>> 6)) & 63]);
            byteArrayOutputStream.write(bArr3[b11 & 63]);
        }
        String strFromByteArray = Strings.fromByteArray(byteArrayOutputStream.toByteArray());
        return z11 ? strFromByteArray.substring(0, 22) : strFromByteArray.substring(0, strFromByteArray.length() - 1);
    }
    ...
    public static String generate(char[] cArr, byte[] bArr, int i11) {
        ...
        String strCreateBcryptString = createBcryptString(bArr2, bArr, i11);

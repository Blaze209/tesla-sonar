...
public class Chacha20Poly1305 implements TlsCipher {
    ...
    public byte[] encodePlaintext(long j11, short s11, byte[] bArr, int i11, int i12) {
        KeyParameter keyParameterInitRecord = initRecord(this.encryptCipher, true, j11, this.encryptIV);
        byte[] bArr2 = new byte[i12 + 16];
        this.encryptCipher.processBytes(bArr, i11, i12, bArr2, 0);
        byte[] bArrCalculateRecordMAC = calculateRecordMAC(keyParameterInitRecord, getAdditionalData(j11, s11, i12), bArr2, 0, i12);
        System.arraycopy(bArrCalculateRecordMAC, 0, bArr2, i12, bArrCalculateRecordMAC.length);
        return bArr2;
    }
    ...
    protected void updateRecordMACLength(Mac mac, int i11) {
        byte[] bArrLongToLittleEndian = Pack.longToLittleEndian(((long) i11) & 4294967295L);

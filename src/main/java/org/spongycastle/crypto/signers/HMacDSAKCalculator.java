...
public class HMacDSAKCalculator implements DSAKCalculator {
    ...
    private BigInteger bitsToInt(byte[] bArr) {
        ...
        return bArr.length * 8 > this.f99611n.bitLength() ? bigInteger.shiftRight((bArr.length * 8) - this.f99611n.bitLength()) : bigInteger;
    }
    ...
    public BigInteger nextK() {
        int iBitLength = (this.f99611n.bitLength() + 7) / 8;
        byte[] bArr = new byte[iBitLength];
        while (true) {
            int i11 = 0;
            while (i11 < iBitLength) {
                HMac hMac = this.hMac;
                byte[] bArr2 = this.V;
                hMac.update(bArr2, 0, bArr2.length);
                this.hMac.doFinal(this.V, 0);
                int iMin = Math.min(iBitLength - i11, this.V.length);
                System.arraycopy(this.V, 0, bArr, i11, iMin);
                i11 += iMin;
            }
            BigInteger bigIntegerBitsToInt = bitsToInt(bArr);
            if (bigIntegerBitsToInt.compareTo(ZERO) > 0 && bigIntegerBitsToInt.compareTo(this.f99611n) < 0) {

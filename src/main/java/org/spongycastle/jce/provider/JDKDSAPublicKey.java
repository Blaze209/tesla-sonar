...
public class JDKDSAPublicKey implements DSAPublicKey {
    ...
    private BigInteger f99651y;

    JDKDSAPublicKey(DSAPublicKeySpec dSAPublicKeySpec) {
        this.f99651y = dSAPublicKeySpec.getY();
    ...
    private void readObject(ObjectInputStream objectInputStream) {
        this.f99651y = (BigInteger) objectInputStream.readObject();
        this.dsaSpec = new DSAParameterSpec((BigInteger) objectInputStream.readObject(), (BigInteger) objectInputStream.readObject(), (BigInteger) objectInputStream.readObject());
    }

    private void writeObject(ObjectOutputStream objectOutputStream) throws IOException {
        objectOutputStream.writeObject(this.f99651y);
    ...
    public byte[] getEncoded() {
        try {
            ...
            return dSAParams == null ? new SubjectPublicKeyInfo(new AlgorithmIdentifier(X9ObjectIdentifiers.id_dsa), new ASN1Integer(this.f99651y)).getEncoded("DER") : new SubjectPublicKeyInfo(new AlgorithmIdentifier(X9ObjectIdentifiers.id_dsa, new DSAParameter(dSAParams.getP(), this.dsaSpec.getQ(), this.dsaSpec.getG())), new ASN1Integer(this.f99651y)).getEncoded("DER");
    ...
    JDKDSAPublicKey(DSAPublicKey dSAPublicKey) {
        this.f99651y = dSAPublicKey.getY();
        this.dsaSpec = dSAPublicKey.getParams();
    }

    JDKDSAPublicKey(DSAPublicKeyParameters dSAPublicKeyParameters) {
        this.f99651y = dSAPublicKeyParameters.getY();
    ...
    JDKDSAPublicKey(BigInteger bigInteger, DSAParameterSpec dSAParameterSpec) {
        this.f99651y = bigInteger;
        this.dsaSpec = dSAParameterSpec;
    }

    JDKDSAPublicKey(SubjectPublicKeyInfo subjectPublicKeyInfo) {
        try {
            this.f99651y = ((ASN1Integer) subjectPublicKeyInfo.parsePublicKey()).getValue();

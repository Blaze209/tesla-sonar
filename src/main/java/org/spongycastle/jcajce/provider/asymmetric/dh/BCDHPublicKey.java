...
public class BCDHPublicKey implements DHPublicKey {
    ...
    private BigInteger f99618y;

    BCDHPublicKey(DHPublicKeySpec dHPublicKeySpec) {
        this.f99618y = dHPublicKeySpec.getY();
        this.dhSpec = new DHParameterSpec(dHPublicKeySpec.getP(), dHPublicKeySpec.getG());
        this.dhPublicKey = new DHPublicKeyParameters(this.f99618y, new DHParameters(dHPublicKeySpec.getP(), dHPublicKeySpec.getG()));
    }
    ...
    BCDHPublicKey(DHPublicKey dHPublicKey) {
        this.f99618y = dHPublicKey.getY();
        this.dhSpec = dHPublicKey.getParams();
        this.dhPublicKey = new DHPublicKeyParameters(this.f99618y, new DHParameters(this.dhSpec.getP(), this.dhSpec.getG()));
    }

    BCDHPublicKey(DHPublicKeyParameters dHPublicKeyParameters) {
        this.f99618y = dHPublicKeyParameters.getY();
    ...
    public BCDHPublicKey(SubjectPublicKeyInfo subjectPublicKeyInfo) {
        ...
        try {
            this.f99618y = ((ASN1Integer) subjectPublicKeyInfo.parsePublicKey()).getValue();
            ...
            if (!algorithm.equals(PKCSObjectIdentifiers.dhKeyAgreement) && !isPKCSParam(aSN1Sequence)) {
                if (algorithm.equals(X9ObjectIdentifiers.dhpublicnumber)) {
                    ...
                    if (validationParams != null) {
                        this.dhPublicKey = new DHPublicKeyParameters(this.f99618y, new DHParameters(domainParameters.getP(), domainParameters.getG(), domainParameters.getQ(), domainParameters.getJ(), new DHValidationParameters(validationParams.getSeed(), validationParams.getPgenCounter().intValue())));
                        return;
                    } else {
                        this.dhPublicKey = new DHPublicKeyParameters(this.f99618y, new DHParameters(domainParameters.getP(), domainParameters.getG(), domainParameters.getQ(), domainParameters.getJ(), (DHValidationParameters) null));
                        return;
                    }
                }

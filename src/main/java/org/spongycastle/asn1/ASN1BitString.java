...
public abstract class ASN1BitString extends ASN1Primitive implements ASN1String {
    ...
    protected static byte[] derForm(byte[] bArr, int i11) {
        byte[] bArrClone = Arrays.clone(bArr);
        if (i11 > 0) {
            int length = bArr.length - 1;
            bArrClone[length] = (byte) ((255 << i11) & bArrClone[length]);
        }
        return bArrClone;
    }

    static ASN1BitString fromInputStream(int i11, InputStream inputStream) throws IOException {
        if (i11 < 1) {
        ...
        if (i13 != 0) {
            if (Streams.readFully(inputStream, bArr) != i13) {
                throw new EOFException("EOF encountered in middle of BIT STRING");
            }
            if (i12 > 0 && i12 < 8) {
                byte b11 = bArr[i11 - 2];
                if (b11 != ((byte) ((255 << i12) & b11))) {
                    return new DLBitString(bArr, i12);
                }
            }
        }
        return new DERBitString(bArr, i12);
    }
    ...
    public int intValue() {
        ...
        if (i11 > 0 && bArrDerForm.length <= 4) {
            bArrDerForm = derForm(bArrDerForm, i11);
        }
        int i12 = 0;
        for (int i13 = 0; i13 != bArrDerForm.length && i13 != 4; i13++) {
            i12 |= (bArrDerForm[i13] & 255) << (i13 * 8);
        }
        return i12;
    }

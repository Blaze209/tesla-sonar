...
public class JcaPublicKeyConverter {
    ...
    private ECParameterSpec getParams(ECDSAPublicKey eCDSAPublicKey) {
        ...
        ECPoint eCPointDecodePoint = fp2.decodePoint(eCDSAPublicKey.getBasePointG());
        return new ECParameterSpec(convertCurve(fp2), new java.security.spec.ECPoint(eCPointDecodePoint.getAffineXCoord().toBigInteger(), eCPointDecodePoint.getAffineYCoord().toBigInteger()), eCDSAPublicKey.getOrderOfBasePointR(), eCDSAPublicKey.getCofactorF().intValue());
    }
    ...
    public PublicKey getKey(PublicKeyDataObject publicKeyDataObject) throws EACException {

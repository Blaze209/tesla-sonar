...
public class EnvelopedDataHelper {
    ...
    byte[] calculateDerivedKey(int i11, char[] cArr, AlgorithmIdentifier algorithmIdentifier, int i12) throws CMSException {
        PBKDF2Params pBKDF2Params = PBKDF2Params.getInstance(algorithmIdentifier.getParameters());
        try {
            return (i11 == 0 ? this.helper.createSecretKeyFactory("PBKDF2with8BIT") : this.helper.createSecretKeyFactory((String) PBKDF2_ALG_NAMES.get(pBKDF2Params.getPrf()))).generateSecret(new PBEKeySpec(cArr, pBKDF2Params.getSalt(), pBKDF2Params.getIterationCount().intValue(), i12)).getEncoded();
    ...
    public Cipher createContentCipher(final Key key, final AlgorithmIdentifier algorithmIdentifier) {
        return (Cipher) execute(new JCECallback() { // from class: org.spongycastle.cms.jcajce.EnvelopedDataHelper.1
            ...
            public Object doInJCE() throws NoSuchAlgorithmException, InvalidKeyException, CMSException, InvalidAlgorithmParameterException {
                ...
                if (parameters == null || (parameters instanceof ASN1Null)) {
                    if (id2.equals(CMSAlgorithm.DES_CBC.getId()) || id2.equals(CMSEnvelopedGenerator.DES_EDE3_CBC) || id2.equals(CMSEnvelopedGenerator.IDEA_CBC) || id2.equals(CMSEnvelopedGenerator.CAST5_CBC)) {
    ...
    Mac createContentMac(final Key key, final AlgorithmIdentifier algorithmIdentifier) {
        return (Mac) execute(new JCECallback() { // from class: org.spongycastle.cms.jcajce.EnvelopedDataHelper.2
            @Override // org.spongycastle.cms.jcajce.EnvelopedDataHelper.JCECallback
            public Object doInJCE() throws InvalidKeyException, CMSException, InvalidAlgorithmParameterException {
        ...
        if (algorithmParameterSpec instanceof RC2ParameterSpec) {
            ...
            if (effectiveKeyBits != -1) {
                if (effectiveKeyBits < 256) {
                    i11 = effectiveKeyBits;
                    i11 = rc2Table[effectiveKeyBits];
                }
                i11 = effectiveKeyBits;
                return new AlgorithmIdentifier(aSN1ObjectIdentifier, new RC2CBCParameter(i11, rC2ParameterSpec.getIV()));
            }

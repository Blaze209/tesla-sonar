...
public class BcPasswordRecipientInfoGenerator extends PasswordRecipientInfoGenerator {
    ...
    protected byte[] calculateDerivedKey(int i11, AlgorithmIdentifier algorithmIdentifier, int i12) throws CMSException {
        PBKDF2Params pBKDF2Params = PBKDF2Params.getInstance(algorithmIdentifier.getParameters());
        byte[] bArrPKCS5PasswordToBytes = i11 == 0 ? PBEParametersGenerator.PKCS5PasswordToBytes(this.password) : PBEParametersGenerator.PKCS5PasswordToUTF8Bytes(this.password);
        try {

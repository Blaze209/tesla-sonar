...
public class ComputeInField {
    ...
    private void substitute() {
        ...
        for (int length = this.A.length - 2; length >= 0; length--) {
            ...
            for (int length2 = sArr4.length - 1; length2 > length; length2--) {
                sAddElem = GF2Field.addElem(sAddElem, GF2Field.multElem(this.A[length][length2], this.f99842x[length2]));
            }
            short sInvElem2 = GF2Field.invElem(this.A[length][length]);
            if (sInvElem2 == 0) {
                throw new RuntimeException("Not solvable equation system");
            }
            this.f99842x[length] = GF2Field.multElem(sAddElem, sInvElem2);
        }
    }
    ...
    public short[] solveEquation(short[][] sArr, short[] sArr2) {
        try {
            ...
            this.f99842x = new short[sArr.length];
            for (int i11 = 0; i11 < sArr.length; i11++) {
                for (int i12 = 0; i12 < sArr[0].length; i12++) {

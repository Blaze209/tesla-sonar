...
public class BigDecimalPolynomial {
    BigDecimal[] coeffs;
    private static final BigDecimal ZERO = new BigDecimal(WebrtcBuildVersion.maint_version);
    private static final BigDecimal ONE_HALF = new BigDecimal("0.5");

    BigDecimalPolynomial(int i11) {
    ...
    private BigDecimal[] copyOfRange(BigDecimal[] bigDecimalArr, int i11, int i12) {
        ...
        if (bigDecimalArr.length - i11 < length) {
            length = bigDecimalArr.length - i11;
        }
        System.arraycopy(bigDecimalArr, i11, bigDecimalArr2, 0, length);
        return bigDecimalArr2;
    }

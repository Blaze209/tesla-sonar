...
public class X509CRLEntryHolder {
    ...
    private GeneralNames f99410ca;
    private TBSCertList.CRLEntry entry;

    X509CRLEntryHolder(TBSCertList.CRLEntry cRLEntry, boolean z11, GeneralNames generalNames) {
        Extension extension;
        this.entry = cRLEntry;
        this.f99410ca = generalNames;
        if (z11 && cRLEntry.hasExtensions() && (extension = cRLEntry.getExtensions().getExtension(Extension.certificateIssuer)) != null) {
            this.f99410ca = GeneralNames.getInstance(extension.getParsedValue());
        }
    }

    public GeneralNames getCertificateIssuer() {
        return this.f99410ca;
    }

...
public class GodotInputHandler implements InputManagerCompat.InputDeviceListener {
    ...
    public boolean onGenericMotionEvent(MotionEvent motionEvent) {
        if ((motionEvent.getSource() & 16777232) == 16777232 && motionEvent.getAction() == 2) {
            final int iFindJoystickDevice = findJoystickDevice(motionEvent.getDeviceId());
            if (iFindJoystickDevice > -1) {
                Joystick joystick = this.joysticksDevices.get(iFindJoystickDevice);
                for (final int i11 = 0; i11 < joystick.axes.size(); i11++) {
                    ...
                    queueEvent(new Runnable() { // from class: org.godotengine.godot.input.GodotInputHandler.5
                        ...
                        public void run() {
                            GodotLib.joyaxis(iFindJoystickDevice, i11, axisValue);
                        }
                    });
                }
                for (int i12 = 0; i12 < joystick.hats.size(); i12 += 2) {
                    final int iRound = Math.round(motionEvent.getAxisValue(joystick.hats.get(i12).getAxis()));
                    final int iRound2 = Math.round(motionEvent.getAxisValue(joystick.hats.get(i12 + 1).getAxis()));
                    queueEvent(new Runnable() { // from class: org.godotengine.godot.input.GodotInputHandler.6
                        @Override // java.lang.Runnable
                        public void run() {
                            GodotLib.joyhat(iFindJoystickDevice, iRound, iRound2);
                        }
                    });
                }
                return true;
            ...
            final int iRound3 = Math.round(motionEvent.getX());
            final int iRound4 = Math.round(motionEvent.getY());
            final int action = motionEvent.getAction();
            queueEvent(new Runnable() { // from class: org.godotengine.godot.input.GodotInputHandler.7
                @Override // java.lang.Runnable
                public void run() {
                    GodotLib.hover(action, iRound3, iRound4);
                }
            });
            return true;
        }
    ...
    public void onInputDeviceRemoved(int i11) {
        final int iFindJoystickDevice = findJoystickDevice(i11);
        if (iFindJoystickDevice > -1) {
            this.joysticksDevices.remove(iFindJoystickDevice);
            queueEvent(new Runnable() { // from class: org.godotengine.godot.input.GodotInputHandler.9
                @Override // java.lang.Runnable
                public void run() {
                    GodotLib.joyconnectionchanged(iFindJoystickDevice, false, "");
                }
            });
        }
    }
    ...
    public boolean onKeyDown(final int i11, KeyEvent keyEvent) {
        ...
        if (!isKeyEvent_GameDevice(keyEvent.getSource())) {
            ...
            final int iFindJoystickDevice = findJoystickDevice(keyEvent.getDeviceId());
            if (iFindJoystickDevice > -1) {
                queueEvent(new Runnable() { // from class: org.godotengine.godot.input.GodotInputHandler.3
                    @Override // java.lang.Runnable
                    public void run() {
                        GodotLib.joybutton(iFindJoystickDevice, godotButton, true);
                    }
                });
            }
        }
        ...
        if (isKeyEvent_GameDevice(keyEvent.getSource())) {
            ...
            if (iFindJoystickDevice > -1) {
                queueEvent(new Runnable() { // from class: org.godotengine.godot.input.GodotInputHandler.1
                    @Override // java.lang.Runnable
                    public void run() {
                        GodotLib.joybutton(iFindJoystickDevice, godotButton, false);
                    }
                });
            }
        } else {
